...
def get_ledger_opening_balance():
    ...
    if auth == 'asoidewfoef':  
       ...
       query = "select case when isnull((isnull(lm.OpDR,0)-ISNULL(lm.OpCR,0)+isnull(t.CumDB,0)-isnull(t.CumCR,0)),0)>0 then Convert(varchar, abs((isnull(lm.OpDR,0)-isnull(lm.OpCr,0)+isnull(t.CumDb,0)-isnull(t.CumCr,0))))+' DR' else Convert(varchar, abs((isnull(lm.OpDR,0)-isnull(lm.OpCr,0)+isnull(t.CumDb,0)-isnull(t.CumCr,0))))+' CR' end as CashBalanceOpening from (select AccountLedger,sum(case when DRCR = 'D' then Amount end) as CumDb,sum(case when DRCR = 'C' then Amount end) as CumCr from T_Account_Transaction where AccountLedger= (%s) group by AccountLedger) t inner join m_ledgermaster lm on t.AccountLedger = lm.tableid"%data['account_ledger']
...
def insert_data_cash_book():
    ...
    if auth == 'asoidewfoef':  
       ...
       query = "exec Usp_T_CashBook 1,0,%s,'','',%s,%s,'',%s,0,'',%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,'','',''"%(data['date'],data['account_head'],data['account_ledger'],data['drcr'],data['standard_description'],data['standard_description'],data['standard_description'],data['standard_description'],data['amount'],data['ledgerhead'],data['ledger1'],data['computername'],data['createdby'],data['createddate'])
       print(query)
       #query = "exec Usp_T_CashBook 1003,0,'2019-03-31','','',1,7125,'','D',0,'','test now','test now','test now','test now',1.00,21,7119,'HIMANSHU','HIMANSHU','2019-03-31','','',''"
       cur.execute(query)
