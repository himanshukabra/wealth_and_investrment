...
def post_auto_transaction_entry():
    ...
    if auth == 'asoidewfoef':  
       ...
       json_final_data = insert_final_data_in_transaction_register(data['dbname'],data['date'],data['broker_id'],0,data['auto_debit_table_id'],'auto_debit_transaction',data['product_id'],data['scrip_id'],data['folio_number'],'Buy',data['units'],data['gross_rate'],data['gross_amount'],0,0,0,data['gross_amount'],'web',data['createdby'],'auto_debit_transaction')
