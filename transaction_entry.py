...
def get_scheme(dbname,product_id):
    ...
    query = "select * from (select Table_Id,ScripName,Product_Id from M_Equity union select id as tableid,bond_name as scripname,3 as Product_Id from M_Bonds union select Table_Id,Scheme_Name as scripname,Product_Id from M_Mutual_Funds union select 0 as TableId, '--Select One--',%s as product_id ) t where Product_Id = %s order by scripname asc"%(product_id,product_id)
...
def get_total_for_temp_transaction(dbname,user_name,computer_name):
    ...
    query = "select isnull(sum(isnull(case when transaction_type='Buy' then gross_amount else -gross_amount end,0)),0) as gross_amount,isnull(sum(isnull(brokerage,0)),0) as total_brokerage, isnull(sum(isnull(stt,0)),0) as stt_amount from t_transaction_api_temp where [user] = '%s' and computer_name = '%s'"%(user_name,computer_name)
    abc = pd.read_sql(query, conn)
...
def get_broker_id_ledger(dbname,broker_id):
    ...
    query = "select cast(account_ledger as integer) as account_ledger from m_broker where id = %s"%broker_id
