...
def get_folios(dbname,scrip_id,product_id):
    ...
    query = "select distinct product_id,scrip_id,folio_number from (select product_id,Bond_Scrip_Id as scrip_id,folio_number from T_Opening_Balance_Products union select product_id,script_id as scrip_id,folio_number from t_transaction_register ) t where folio_number is not null and product_id = %s and scrip_id = %s"%(product_id,scrip_id)
